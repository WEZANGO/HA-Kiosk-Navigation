...
def clean_dashboard(payload: dict, existing: dict | None = None) -> dict:
    ...
    raw_id = str(existing["id"] if existing else payload.get("id", name)).lower()
    ...
    for key, default in DEFAULTS.items():
        values[key] = str(payload.get(key, existing.get(key, default) if existing else default))
    return values
...
<section><h2 id="form-title">New dashboard</h2><form id="editor"><input id="edit-id" type="hidden"><label>Name<input name="name" required placeholder="Morning commute"></label><label>Origin<input name="origin" required placeholder="Home"></label><label>Destination<input name="destination" required placeholder="Office"></label><label>Map appearance<select name="theme"><option value="dark">Dark</option><option value="light">Light</option></select></label><label>Map framing<select name="zoomOut"><option value="0.7">Normal</option><option value="1.2" selected>Spacious</option><option value="1.7">Extra spacious</option></select></label><label>Route width<select name="routeWidth"><option value="8">Slim</option><option value="12" selected>Standard</option><option value="16">Wide</option><option value="22">Extra wide</option></select></label><label>Route border<select name="borderWidth"><option value="4">Thin</option><option value="7" selected>Standard</option><option value="10">Thick</option></select></label><label>Green delay through (%)<input name="greenThreshold" type="number" min="0" max="100" value="10"></label><label>Amber delay through (%)<input name="amberThreshold" type="number" min="0" max="100" value="30"></label><div class="wide"><button type="submit">Save dashboard</button> <button class="secondary" id="cancel" type="button" hidden>Cancel edit</button></div></form></section>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def display(self, identifier: str, compact: bool) -> None:
        try:
            ...
            config = {**DEFAULTS, **dashboards[index], "apiKey": api_key()}
