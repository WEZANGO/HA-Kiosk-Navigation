...
const select=(name,values,selected)=>`<label>${name}<select name="${name}">${values.map(([value,label])=>`<option value="${value}"${value===selected?' selected':''}>${label}</option>`).join('')}</select></label>`;
const input=(name,placeholder)=>`<label>${name}<input name="${name}" placeholder="${placeholder}"></label>`;
const options={
...
for(const [prefix,heading] of [['full','Full-screen presentation'],['compact','Compact presentation']]){
  ...
  anchor.insertAdjacentHTML('afterend', `<h4 class="wide">${heading}</h4>${input(prefix+'Title','Optional title')}${select(prefix+'TitlePosition',options.position,'top')}${select(prefix+'TitleSize',options.size,'medium')}${select(prefix+'TitleBackground',options.background,'rounded')}${select(prefix+'TitleFont',options.font,'system')}${select(prefix+'MetricSize',options.cards,'large')}${select(prefix+'MetricStyle',options.cardStyle,'rounded')}`);
}
</script>"""
